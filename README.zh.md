...
**从任意回答分叉，再从分支里的回答继续分叉。**

这是一个 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)
插件，专门处理可以递归嵌套的隔离追问：先从主对话的任意回答岔出一个侧问；如果侧问
的回答又有不懂之处，就从那条回答继续分叉。每一级只继承自己的祖先链，主任务始终是
一条干净的线。
...
> **主任务 → 一级侧问 → 针对侧问回答的二级侧问 → ……**

- **随时岔出一条侧线。** 一级分支拿到的正好是所选回答当时已有的历史。
- **分支里的回答还能再分支。** 每一层都可以成为新的隔离 fork 点，深度不限。
...
| 做法 | 保留此前相关上下文 | 不污染主任务 | 能从侧问回答再次分叉 | 侧线仍依附根会话 |
| --- | --- | --- | --- | --- |
| 在主对话里继续问 | 是 | 否 | 否 | 不适用 |
| 新开一个对话 | 丢失，或手工复制 | 是 | 否 | 否 |
| 常见的临时侧栏问答 | 视实现而定 | 通常可以 | 通常仍是一条线 | 通常不会 |
| **dsh-nested-followups** | **精确继承祖先链** | **是** | **可以，深度不限** | **是** |

真正的区别不在画布本身，而在于：侧聊里的一条回答也可以成为新的隔离 fork 点。
树只是这套递归侧线的地图，让它们始终可见、依附于根会话，也不会把左侧会话列表塞满。
