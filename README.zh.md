...
折叠采用逐级展开。点击某条回答旁的 **⊖**，可把该锚点下的全部分支收成一个点位；
点击 **⊕** 后会出现每条分支各自的胶囊，再点击某枚胶囊才恢复该分支的消息卡片。
其他胶囊保持折叠，因此同级分支可以独立地部分展开。想把卡片组收回胶囊，可点击虚线框
内侧底边的上箭头热区，也可以使用首张卡片菜单中的 **折叠分支**。

按住 Alt 点击 **⊕** 或胶囊，可以一次深度展开其全部后代；工具栏的 **一键全收** 会把
所有一级锚点组收成点位。收回整组不会清除内部各分支的展开记录，重启后也会按当前会话
恢复布局。折叠状态下，蓝色脉冲表示后代仍在生成，红色表示其中有失败；搜索命中折叠
内容时会自动逐级展开祖先链并定位消息。
