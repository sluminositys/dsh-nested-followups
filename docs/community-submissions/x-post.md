...
Attach `assets/demo.gif`. The animation carries the feature explanation, so do
not add a second feature list to the post.

## Optional two-post thread

Post 1:

```text
Linear agent chats make tiny side questions expensive: ask in the main thread and the detour follows every later request; open a new chat and the useful context is gone.

I built a DSH plugin for the missing third option.
```
